...
<div id="shotmanagement_sort">
    <!-- 신고 된 챌린지만 보기 -->
    <input id="handle" type="checkbox" value="yes" >미완료 처리 인증샷만 보기
    ...
    <select id="shotmanagementorderby">
        <option id="lastest" value="lastest">최신순</option>
        <option id="sirenup" value="sirenup">신고많은순</option>
        <option id="challengesort" value="challengesort">챌린지 별로 보기</option>
    </select>
...
<div id="shotmanagement_shots">
    ...
    */
    ...
    if(!$sql){ // sql문 오류
        ...
        // 데이터가 존재하지 않을 때
        if($total_rows==0){
            echo "<div style='text-align:center'>데이터가 존재하지 않습니다</div>";
        } 
        ?>
        ...
        <button id="handleok" style="float:right" <?php if($total_rows==0){echo "hidden";} ?>>
        처리완료
        </button>
        <br>
    <?php } ?>
    ...
    if($pageNum<=1){//페이지번호가 1보다 작거나 같다면
        ...
        echo "<font size=2><a class='pagelink' href='challengemanagement.php?page=&list={$list}&handleok2={$handleok2}&sirenok={$sirenok}&shotmanagementorderby={$shotmanagementorderby}&management=shotmanagement'> [처음] </a></font>";
    ...
    if($block<=1){
        ...
        echo "<font size=2><a class='pagelink' href='challengemanagement.php?page={$insertpage}&list={$list}&handleok2={$handleok2}&sirenok={$sirenok}&shotmanagementorderby={$shotmanagementorderby}&management=shotmanagement'> 이전 </a></font>";
    }
    ...
    for($j=$b_start_page; $j<=$b_end_page; $j++){
        if($pageNum==$j){
            ...
            echo"<font size=2><a class='pagelink' href='challengemanagement.php?page={$j}&list={$list}&handleok2={$handleok2}&sirenok={$sirenok}&shotmanagementorderby={$shotmanagementorderby}&management=shotmanagement'> {$j} </a></font>";
    ...
    if($block>=$total_block){
        ...
        $temp = $b_end_page+1;
        echo "<font size=2><a class='pagelink' href='challengemanagement.php?page={$temp}&list={$list}&handleok2={$handleok2}&sirenok={$sirenok}&shotmanagementorderby={$shotmanagementorderby}&management=shotmanagement'> 다음 </a></font>";
    }
    ...
    if($pageNum>=$total_page){
        ...
        //그게 아니라면
        echo "<font size=2><a class='pagelink' href='challengemanagement.php?page={$total_page}&list={$list}&handleok2={$handleok2}&sirenok={$sirenok}&shotmanagementorderby={$shotmanagementorderby}&management=shotmanagement'> [마지막] </a></font>";
    }
    ?>
    </div>
